...
import type {
  ...
  AdaptiveNextActionOutput,
  GoalInput,
} from "../core/types";

const logger = new Logger("AdaptivePlannerAgent");
...
export class AdaptivePlannerAgent {
  ...
  public async nextAction(input: AdaptivePlannerInput): Promise<AdaptiveNextActionOutput> {
    ...
    const output = LlmClient.parseJsonResponse<AdaptiveNextActionOutput>(
      response.content
    );
  ...
  private buildUserMessage(input: AdaptivePlannerInput): string {
    return [
      `## Goal`,
      input.goal.goal,
      ``,
  ...
  private validateOutput(output: AdaptiveNextActionOutput): void {
  ...
  private normalizeInstruction(
    output: AdaptiveNextActionOutput,
    snapshot: AomSnapshot
  ): void {
    ...
    const normalized = this.convertUidSyntaxToStableInstruction(
      output.instruction,
      snapshot
    );
  ...
  private convertUidSyntaxToStableInstruction(
    instruction: string,
    snapshot: AomSnapshot
  ): string {
  ...
  private describeNode(node: AomSnapshot["nodes"][number]): string {
    const name =
      node.label ||
      node.name ||
      node.ariaLabel ||
