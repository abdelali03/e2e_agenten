...
const logger = new Logger("AllLlmAgent");
...
export interface AllLlmAgentInput {
  goal: GoalInput;
...
export class AllLlmAgent {
  private readonly llm = new LlmClient();

  public async decide(input: AllLlmAgentInput): Promise<LlmBrowserCommand> {
    ...
    const command = this.normalize(
      LlmClient.parseJsonResponse<Partial<LlmBrowserCommand>>(response.content)
    );
  ...
  private buildUserMessage(input: AllLlmAgentInput): string {
    return [
      `## Goal`,
      input.goal.goal,
      ``,
  ...
  private normalize(raw: Partial<LlmBrowserCommand>): LlmBrowserCommand {
    const status =
      raw.status && ["continue", "complete", "blocked"].includes(raw.status)
        ? raw.status
        : raw.actionType
        ? "continue"
        : "blocked";
