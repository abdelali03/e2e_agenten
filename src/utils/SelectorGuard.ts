export interface SelectorGuardResult {
  ok: boolean;
  target?: string;
  error?: string;
}
...
export function validateMcpActionTarget(
  toolName: string,
  ...
  if (/^\[\s*-?\d+(\.\d+)?\s*,\s*-?\d+(\.\d+)?\s*\]$/.test(trimmed)) {
    return {
      ...
      target: trimmed,
      error:
        "Vision coordinates are not executable MCP targets. Use an MCP ref like e123 or a valid selector/text/role locator.",
    };
  }
  ...
  if (/^\{.*(?:point|box_2d|x|y).*\}$/i.test(trimmed)) {
    return {
      ...
      target: trimmed,
      error:
        "Raw vision JSON is not an executable MCP target. Convert visual hints to MCP refs/selectors first.",
    };
  }
  ...
  return {
    ...
    target: trimmed,
    error:
      "Target is neither an MCP ref nor a supported selector. Use browser_snapshot/perception to resolve a valid target.",
  };
}

function validateFillForm(args: Record<string, unknown>): SelectorGuardResult {
...
function isLikelyValidSelector(target: string): boolean {
  return (
    /^text\s*=/.test(target) ||
    /^role\s*=/.test(target) ||
    /^css\s*=/.test(target) ||
