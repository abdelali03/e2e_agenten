...
const logger = new Logger("EnhancedMcpSnapshotTool");

export interface EnhancedSnapshotResult {
  text: string;
  rawSnapshotText: string;
  context?: EnhancedUiContext;
  contextError?: string;
  isError?: boolean;
}

export interface EnhancedUiContext {
  url: string;
  title: string;
  timestamp: string;
  viewport: {
  ...
  activeElement?: EnhancedElementSummary;
  visibleText: string;
  dialogs: EnhancedElementSummary[];
  overlays: EnhancedElementSummary[];
  forms: EnhancedElementSummary[];
  fields: EnhancedElementSummary[];
  interactiveElements: EnhancedElementSummary[];
  errorsAndAlerts: EnhancedElementSummary[];
  menusAndPopups: EnhancedElementSummary[];
  tablesAndLists: EnhancedElementSummary[];
  componentHints: string[];
  accessibilityWarnings: string[];
  sceneGraph?: GenericSceneGraph;
}

export interface EnhancedElementSummary {
  sceneId?: string;
  selector?: string;
  mcpRef?: string;
  tagName: string;
...
export interface GenericSceneGraph {
  perceptionQuestion?: string;
  nodes: GenericSceneNode[];
  regions: GenericRegion[];
  rankedRegions: GenericRegion[];
  semanticSearch: GenericSemanticSearchResult[];
  observationContract: GenericObservationContract;
}

export interface GenericSceneNode extends EnhancedElementSummary {
  sceneId: string;
  selector: string;
  mcpRef?: string;
  rankText: string;
...
export interface GenericSemanticSearchResult {
  query: string;
  status: "answered" | "not_found" | "ambiguous";
  candidates: Array<{
    sceneId: string;
    mcpRef?: string;
    runtimeSelector?: string;
    ...
    value?: string;
    actions: string[];
    bounds?: EnhancedElementSummary["bounds"];
    score: number;
    reason: string;
  }>;
...
export interface GenericObservationContract {
  question: string;
  requiredOutput: string;
  status: "answered" | "not_found" | "ambiguous" | "needs_visual";
  answerSummary: string;
  recommendedNextObservation?: string;
  blockers: string[];
}

export class EnhancedMcpSnapshotTool {
  public async capture(
    mcp: PlaywrightMcpClient,
    args: Record<string, unknown> = {}
  ): Promise<EnhancedSnapshotResult> {
    const perceptionQuestion =
      typeof args.perceptionQuestion === "string" ? args.perceptionQuestion : undefined;
    ...
    const enhancedContext = context.context
      ? {
          ...context.context,
          sceneGraph: this.buildSceneGraph(
            context.context,
            snapshot.text,
            perceptionQuestion
          ),
  ...
  private async extractUiContext(
    mcp: PlaywrightMcpClient
  ): Promise<{ context?: EnhancedUiContext; error?: string }> {
    try {
  ...
  private parseContext(text: string): EnhancedUiContext | undefined {
    ...
    for (const candidate of candidates) {
      try {
        const parsed = JSON.parse(candidate) as EnhancedUiContext;
  ...
  private extractJsonStringLiteral(text: string): string | undefined {
    try {
  ...
  private buildEnhancedText(
    snapshotText: string,
    context: EnhancedUiContext | undefined,
    contextError: string | undefined,
    perceptionQuestion: string | undefined
  ): string {
    const sections = [
    ...
    if (context) {
      sections.push(
        "",
        "=== UI ACTIVE SURFACE SUMMARY ===",
        JSON.stringify(this.buildSurfaceSummary(context), null, 2),
        "",
        "=== GENERIC PAGE MAP ===",
        JSON.stringify(this.buildGenericPageMap(context, perceptionQuestion), null, 2),
        "",
        "=== GENERIC SCENE GRAPH ===",
        JSON.stringify(context.sceneGraph, null, 2).slice(0, 24_000)
      );
      ...
      sections.push(
        "",
        "=== FULL ENHANCED UI CONTEXT ===",
        JSON.stringify(context, null, 2).slice(0, 18_000)
      );
    } else {
      sections.push(
        JSON.stringify(
    ...
    sections.push(
      "",
      "=== USAGE NOTES ===",
      "- Use MCP refs from the accessibility snapshot for browser_click, browser_fill_form, browser_type, and related actions.",
      "- Use enhanced UI context to understand visible text, layout, dialogs, overlays, active/focused elements, validation errors, forms, custom widgets, and accessibility gaps.",
      "- If a candidate has no MCP ref but includes runtimeSelector, it is a live-session fallback selector injected by the observer and may be used for immediate browser actions.",
      "- runtimeSelector/data-ai-scene-id is transient perception metadata. Never write it into generated tests; test generation must prefer stable role/label/text/testid/user-facing locators.",
      "- If enhanced context shows visible UI that the MCP snapshot does not expose as refs, do not invent refs. Prefer runtimeSelector candidates, keyboard navigation, targeted vision, or another accessible path."
    );

    return sections.join("\n").slice(0, 60_000);
  }

  private buildSurfaceSummary(context: EnhancedUiContext): Record<string, unknown> {
    ...
    return {
      ...
      note:
        "This summary is perception only. Use MCP refs from the accessibility snapshot for execution; if an active surface exists, interact inside it before background controls.",
    };
  }

  private buildGenericPageMap(
    context: EnhancedUiContext,
    perceptionQuestion?: string
  ): Record<string, unknown> {
    const graph =
      context.sceneGraph ?? this.buildSceneGraph(context, "", perceptionQuestion);
    const regions = graph.rankedRegions;
    const focusedRegion =
      regions.find((region) => region.priority === "active-surface") ||
      regions.find((region) => region.priority === "main-work-area") ||
      regions[0];

    return {
      purpose:
        "Generic multi-modal perception map. Use it to choose the relevant region before requesting deeper perception.",
      url: context.url,
      title: context.title,
  ...
  private buildSceneGraph(
    ...
    const observationContract = this.buildObservationContract(
      perceptionQuestion,
      semanticSearch,
      rankedRegions,
      context
  ...
  private parseMcpSnapshotRefs(snapshotText: string): Array<{
    ref: string;
    role?: string;
    name?: string;
    text?: string;
  ...
  private buildSceneNodes(
    context: EnhancedUiContext,
    mcpNodes: ReturnType<EnhancedMcpSnapshotTool["parseMcpSnapshotRefs"]>
  ): GenericSceneNode[] {
    const rawNodes = [
      ...context.dialogs,
      ...context.overlays,
    ...
    const unique = new Map<string, EnhancedElementSummary>();
  ...
  private findBestMcpMatch(
    node: EnhancedElementSummary,
    mcpNodes: ReturnType<EnhancedMcpSnapshotTool["parseMcpSnapshotRefs"]>
  ): ReturnType<EnhancedMcpSnapshotTool["parseMcpSnapshotRefs"]>[number] | undefined {
    let best:
      | {
          item: ReturnType<EnhancedMcpSnapshotTool["parseMcpSnapshotRefs"]>[number];
          score: number;
        }
      | undefined;
    const nodeText = `${node.label || ""} ${node.ariaLabel || ""} ${node.text || ""} ${
      node.value || ""
    }`.toLowerCase();
    ...
    for (const item of mcpNodes) {
      ...
      if (node.bounds && item.bounds) {
        ...
        if (distance < 12) score += 80;
        else if (distance < 30) score += 45;
        else if (distance < 80) score += 15;
      }
      ...
      if (item.name && nodeText.includes(item.name.toLowerCase())) score += 30;
  ...
  private inferActions(node: EnhancedElementSummary): string[] {
  ...
  private buildRegionsFromNodes(
    ...
    const pushRegion = (
      ...
      kind: string,
      summary: string,
      bounds: GenericRegion["bounds"],
      regionNodes: GenericSceneNode[],
      reasons: string[]
    ) => {
    ...
    const activeSurfaceNodes = nodes.filter((node) =>
      node.bounds
        ? [...context.dialogs, ...context.overlays, ...context.menusAndPopups].some(
            (surface) => surface.bounds && this.isInside(node.bounds!, surface.bounds)
          )
        : false
    );
    const activeSurfaceBounds =
      context.dialogs[0]?.bounds || context.overlays[0]?.bounds || context.menusAndPopups[0]?.bounds;
    pushRegion(
      "active-surface",
      "top-layer",
      "Currently active overlay/dialog/menu surface.",
      activeSurfaceBounds,
      activeSurfaceNodes,
      ["visible top-layer surface should usually be handled before background UI"]
    );
    ...
    pushRegion(
      ...
      mainBounds,
      nodes.filter((node) => node.bounds && this.isInside(node.bounds, mainBounds)),
      ["right/central visible work area"]
    );
    ...
    pushRegion(
      "navigation-or-left-rail",
      "left-region",
      "Left side interactive region.",
      leftBounds,
      nodes.filter((node) => node.bounds && this.isInside(node.bounds, leftBounds)),
      ["left-side dense interactive area"]
    );
    ...
    pushRegion(
      ...
      topBounds,
      nodes.filter((node) => node.bounds && this.isInside(node.bounds, topBounds)),
      ["top horizontal controls"]
    );

    const formNodes = nodes.filter((node) =>
      ["field", "action"].includes(this.regionKind(node)) || node.actions.some((action) => ["type", "fill"].includes(action))
    );
    pushRegion(
      "form-cluster",
      "field-action-cluster",
      "Visible editable fields and nearby actions.",
      this.unionBounds(formNodes),
      formNodes,
      ["cluster of editable fields/actions"]
    );

    const denseNodes = nodes.filter((node) =>
      (node.componentHints || []).some((hint) =>
        /grid|table|list|data|calendar|picker/.test(hint)
      ) || ["grid", "table", "list"].includes(node.role || "")
    );
    pushRegion(
      "dense-data-region",
      "grid-list-or-repeated-content",
      "Dense data/list/grid-like region.",
      this.unionBounds(denseNodes),
      denseNodes,
      ["dense repeated structure"]
    );

    return regions.slice(0, 12);
  }

  private unionBounds(nodes: GenericSceneNode[]): GenericRegion["bounds"] {
    const bounds = nodes.map((node) => node.bounds).filter((item): item is NonNullable<typeof item> => Boolean(item));
  ...
  private rankRegions(
    ...
    return regions
      .map((region) => {
        ...
        if (region.priority === "main-work-area") score += 40;
  ...
  private buildSemanticSearch(
    nodes: GenericSceneNode[],
    perceptionQuestion?: string
  ): GenericSemanticSearchResult[] {
    ...
    const candidates = nodes
      .map((node) => {
        ...
        if (node.mcpRef) score += 15;
        if (node.actions.length > 0) score += 10;
    ...
    const status =
      candidates.length === 0 ? "not_found" : candidates.length === 1 || candidates[0].score - candidates[1].score > 20 ? "answered" : "ambiguous";
    ...
    return [
      {
        ...
        candidates: candidates.map((item) =>
          this.toCandidate(item.node, item.score, "matched perception question")
        ),
  ...
  private toCandidate(
    ...
    score: number,
    reason: string
  ): GenericSemanticSearchResult["candidates"][number] {
    return {
      sceneId: node.sceneId,
      mcpRef: node.mcpRef,
      runtimeSelector: node.mcpRef ? undefined : node.selector,
  ...
  private buildObservationContract(
    perceptionQuestion: string | undefined,
    semanticSearch: GenericSemanticSearchResult[],
    rankedRegions: GenericRegion[],
    context: EnhancedUiContext
  ): GenericObservationContract {
  ...
  private buildGenericRegions(
    ...
    const leftControls = context.interactiveElements.filter(
      (item) => item.bounds && item.bounds.x < context.viewport.width * 0.35
    );
  ...
  private childrenInside(
    context: EnhancedUiContext,
    container: EnhancedElementSummary
  ): Array<Record<string, unknown>> {
    if (!container.bounds) {
      return [];
    }
  ...
  private compactElement(item: EnhancedElementSummary): Record<string, unknown> {
    return {
      kind: this.regionKind(item),
      role: item.role,
  ...
  private regionKind(item: EnhancedElementSummary): string {
    const hints = item.componentHints ?? [];
    if (hints.includes("dialog") || item.role === "dialog") return "dialog-or-modal";
    if (hints.includes("select-popup") || item.role === "listbox") return "menu-or-popup";
    if (hints.includes("table-or-grid") || item.role === "grid") return "grid-or-list";
    if (item.role === "textbox" || item.tagName === "input" || item.tagName === "textarea") return "field";
    if (item.role === "button" || item.tagName === "button") return "action";
    return item.role || item.tagName;
  }

  private looksLikePrimaryAction(item: EnhancedElementSummary): boolean {
  ...
  private elementSummaryText(item: EnhancedElementSummary): string {
    return [
  ...
  private getExtractionCode(): string {
    return `async (page) => {
