...
test_1.test.describe("V3 hard timeouts", () => {
    ...
    (0, test_1.test)("observe() enforces timeoutMs", async () => {
        // Tiny timeout to force the race to hit the timeout branch
        await (0, test_1.expect)(v3.observe("find something", { timeout: 5 })).rejects.toThrow(/timed out/i);
    });
    (0, test_1.test)("extract() enforces timeoutMs", async () => {
        const schema = zod_1.z.object({ title: zod_1.z.string().optional() });
        await (0, test_1.expect)(v3.extract("Extract title", schema, { timeout: 5 })).rejects.toThrow(/timed out/i);
    });
